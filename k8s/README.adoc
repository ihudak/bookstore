...
==== use preinstrumented images for Intel/AMD architecture, books namespace (default ns is bookstore):
```
./preset_deployment.sh -gyes -ax64 -n books
```

==== use preinstrumented images for ARMv8 architecture, books namespace (default ns is bookstore):
```
